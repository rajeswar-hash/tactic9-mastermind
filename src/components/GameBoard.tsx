...
export default function GameBoard({ board, winningCells, lastMove, gameOver, onCellClick }: GameBoardProps) {
  return (
    <div className="flex justify-center my-6 overflow-x-auto p-4 bg-background rounded-2xl border border-border">
      <div
        className="grid gap-[6px] p-3 rounded-2xl bg-card-light"
        ...
        {board.map((cell, i) => {
          ...
          return (
            <button
              ...
              className={`
                w-[55px] h-[55px] rounded-[10px] flex items-center justify-center
                text-2xl font-extrabold border-2 transition-all duration-200
                ...
                ${isLast && !isWinner ? 'animate-[lastMovePulse_1.5s_ease-in-out_infinite]' : ''}
                ${cell && !isWinner && !isLast ? 'animate-[popIn_0.3s_ease]' : ''}
              `}
              style={{
                textShadow: cell === 'X'
                ...
                boxShadow: isWinner
                  ...
                  : isLast
                  ? cell === 'X'
                    ? '0 0 25px 5px hsl(var(--primary) / 0.6)'
                    : '0 0 25px 5px hsl(var(--secondary) / 0.6)'
                  : undefined,
              }}
            >
              {cell}
